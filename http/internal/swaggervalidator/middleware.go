...
func (m *validatingMiddleware) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		// Validation errors are formatted with a lot of whitespace, so just fmt.Println this out for developers to read.
		fmt.Println("SWAGGER REQUEST VALIDATION ERROR:", err.Error())
	}
	...
	if err := t.ValidateResponse(r.Context(), reqVI); err != nil {
		// Validation errors are formatted with a lot of whitespace, so just fmt.Println this out for developers to read.
		fmt.Println("SWAGGER RESPONSE VALIDATION ERROR:", err.Error())
	}
}
